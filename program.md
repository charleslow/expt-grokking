...
2. **Agree on a target**: The user will specify a specific figure or table from the paper to replicate. Record the exact target values in `insights.md` under a "Paper Target" section.
3. **Agree on a run tag**: Propose a tag based on the paper and today's date (e.g., `paper_name_mar5`). The branch `replication/<tag>` must not already exist.
4. **Create the branch**: `git checkout -b replication/<tag>` from the current main branch.
5. **Read the in-scope files**: Read the existing codebase to understand the current setup. Identify which files are for data preparation, which are for training, and which are for evaluation.
6. **Set up the environment**: Use `uv` for Python packaging. Create a `pyproject.toml` if one doesn't exist. Install dependencies with `uv pip install` or `uv sync`. Pin the environment so results are reproducible.
7. **Initialize tracking**: Create a `.replication_logs/` directory with empty `insights.md` and `ideas_queue.md`. In `insights.md`, add:
    ```markdown
    ## Paper Target
    <exact values from the figure/table the user specified>

    ## Current Best
    commit: (none yet)
    metric: (none)
    ```
8. **Confirm and go**: Confirm setup looks good and the target (from the paper's table/figure) is clearly defined.

Once you get confirmation, kick off the experimentation.

## Experimentation

The goal is to get as close to the target results (from the user-specified table or figure in the paper) as possible.

**Run command** (always run in detached mode so you are not blocked):
```shell
nohup uv run python train.py > run.log 2>&1 &
```
*Note: Adjust `train.py` to the actual training command for the repository.*
...
**One change at a time**: Each commit should change exactly one variable or hypothesis. Never change multiple things at once, so you can attribute improvements or regressions to a specific change.
...
```text
.replication_logs/
  results.tsv              # Master log (append-only TSV, columns defined per experiment)
  insights.md              # Your validated insights + CURRENT BEST + PAPER TARGET
...
**results.tsv**: Define the columns at experiment start based on what the target figure/table requires. The first row should be a header. Always include at minimum a `commit` and `description` column; the remaining columns should capture whatever metrics are needed to compare against the paper's target.
